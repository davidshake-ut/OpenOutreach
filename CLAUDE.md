...
### Docker (Recommended)
```bash
docker run --pull always -it -p 5900:5900 --user "$(id -u):$(id -g)" -v ./assets:/app/assets <your-registry>/openoutreach:latest  # run from pre-built image (requires custom build)
...
- **`ml/hub.py`** — Partner campaign hub (disabled in this fork). `get_kit()` returns `None` (partner campaigns feature disabled). `import_partner_campaign(kit_config)` creates/updates a partner `Campaign` with `is_partner=True` if kit was loaded. Cached after first attempt.
...
- **`.github/workflows/deploy.yml`** — On push to `master` or version tags (`v*`): runs tests, then builds and pushes the production Docker image to your registry. Configure with your own container registry credentials.
